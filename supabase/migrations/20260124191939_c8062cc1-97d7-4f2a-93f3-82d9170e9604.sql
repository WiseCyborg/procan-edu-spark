...
DROP FUNCTION IF EXISTS public.approve_dispensary_application(UUID, INTEGER, UUID);

-- Recreate with robust error handling and portable token generation
...
BEGIN
  ...
  -- Generate registration token using encode + gen_random_bytes (pgcrypto now enabled)
...
GRANT EXECUTE ON FUNCTION public.approve_dispensary_application(UUID, INTEGER, UUID) TO authenticated;