-- Fix column ambiguity in approve_dispensary_application function
-- Rename local variable 'registration_token' to 'reg_token' to avoid conflict with table column
...
DECLARE
  ...
  reg_token TEXT;  -- Renamed from registration_token to avoid ambiguity
BEGIN
  ...
  -- Generate registration token and URL (using renamed variable)
  reg_token := ENCODE(GEN_RANDOM_BYTES(32), 'hex');
  registration_url := 'https://zhmpwczrvitomsxjwpzc.supabase.co/manager-registration?token=' || reg_token;
