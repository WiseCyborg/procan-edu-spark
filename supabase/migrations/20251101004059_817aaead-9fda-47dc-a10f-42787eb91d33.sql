-- Fix security: Add search_path to rate limit functions
DROP FUNCTION IF EXISTS clear_user_rate_limits(uuid, text);
DROP FUNCTION IF EXISTS get_active_rate_limits();

-- Create function to clear rate limits for a user (with search_path)
...
-- Create function to view active rate limits with time remaining (with search_path)
