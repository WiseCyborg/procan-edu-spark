-- Fix ambiguous column reference in approve_dispensary_application
-- Issue: UPDATE statement references 'registration_token' which is ambiguous (column vs variable)
...
BEGIN
  ...
  UPDATE public.dispensary_applications AS da
  SET 
    application_status = 'approved',
    reviewed_by = verified_user_id,
    reviewed_at = now(),
    organization_id = new_org_id,
    registration_token = approve_dispensary_application.registration_token,
    registration_token_expires_at = NOW() + INTERVAL '7 days',
    admin_notes = COALESCE(da.admin_notes, '') || 
...
COMMENT ON FUNCTION public.approve_dispensary_application IS 
  'Approves dispensary applications with atomic seat allocation. Fixed ambiguous column reference in UPDATE statement.';