-- Fix get_access_snapshot with CORRECT column names:
-- organization_id ✓, status (not is_active) ✓, member_type ✓
...
BEGIN
  ...
  IF v_course.payment_required = true THEN
    ...
    -- Org membership entitlement (CORRECT: status = 'active', not is_active)
...
-- Ensure RPC is callable
GRANT EXECUTE ON FUNCTION public.get_access_snapshot(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_access_snapshot(uuid) TO anon;